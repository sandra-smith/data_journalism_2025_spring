...
vote_changes <- left_join(votes_2020, votes_2024, by = c("election_district_precinct", "county_name", "party")) |>
  mutate(
    differences = total_votes_2024 - total_votes_2020
  )

#Seperate by party 
democratic_changes <- vote_changes |>
  filter(party == "DEM") |>
  select(election_district_precinct, county_name, democratic_changes = differences)

republican_changes <- vote_changes |>
  filter(party == "REP") |>
  select(election_district_precinct, republican_changes = differences)

#Join parties (asked ChatGPT for help with the full join)
party_changes <- full_join(democratic_changes, republican_changes, by = "election_district_precinct") |>
  arrange(desc(abs(democratic_changes) + abs(republican_changes)))
