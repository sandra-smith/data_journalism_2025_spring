...
```{r}
# Party registrations
new_party_registrations2024 <- read_csv("new_party_registrations.csv", col_names=c("METHOD", "DEM", "REP", "NLM", "LIB", "GRN", "UNAF", "OTH", "TOTAL", "DUPS") )

glimpse(new_party_registrations2024)
```

```{r}
# Removals

removals2024 <- read_csv("removals.csv")

colnames(removals2024) <- c("SUMMARY", "DEM", "REP", "NLM", "LIB", "GRN", "UNAF", "OTH", "TOTAL")
...
registration_changes2024 <- read_csv("registration_changes.csv", col_names = c("COUNTY", "ADDRESS_CHANGES", "NAME_CHANGES", "DEM", "REP", "NLM", "LIB", "GRN", "UNAF", "OTH", "TOTAL"))
...
current_registrations2024 <- read_csv("current_registrations.csv", col_names=c("DEM", "REP", "NLM", "LIB","GRN", "UNAF","OTH", "TOTAL", "CONFMAILING", "INACTIVE"))
...
new_party_registrations2022 <- read_csv("new_party_registrations2.csv", col_names=c("METHOD", "DEM", "REP", "NLM", "LIB", "GRN", "UNAF", "OTH", "TOTAL", "DUPS") )
...
removals2022 <- read_csv("removals2.csv", col_names = c("SUMMARY", "DEM", "REP", "NLM", "LIB", "GRN", "UNAF", "OTH", "TOTAL"))
...
registration_changes2022 <- read_csv("registration_changes2.csv", col_names = c("COUNTY", "ADDRESS_CHANGES", "NAME_CHANGES", "DEM", "REP", "NLM", "LIB", "GRN", "UNAF", "OTH", "TOTAL"))
...
current_registrations2022 <- read_csv("current_registrations2.csv", col_names = c("DEM", "REP", "NLM", "LIB", "GRN", "UNAF", "OTH", "TOTAL", "CONFMAILING", "INACTIVE"))
...
A1. Democratic registration dropped by 3.03% from 2022-2024, Republican registration also dropped by 0.62% and unaffiliated voter registration increased by 7.98%. The most interesting change is the rise in unaffiliated voter registration. This would be the best lede of the story. (I asked chatgpt for help on the percent change section.) 

```{r}

sept_2022 <- current_registrations2022 |>
  mutate(dem_percent = DEM / TOTAL * 100,
         rep_percent = REP / TOTAL * 100,
         unaff_percent = UNAF / TOTAL * 100)

sept_2024 <- current_registrations2024 |>
  mutate(dem_percent = DEM / TOTAL * 100,
         rep_percent = REP / TOTAL * 100,
         unaff_percent = UNAF / TOTAL * 100)

percent_change <- sept_2024 |>
  mutate(dem_change = (dem_percent - sept_2022$dem_percent) / sept_2022$dem_percent * 100,
         rep_change = (rep_percent - sept_2022$rep_percent) / sept_2022$rep_percent * 100,
         unaff_change = (unaff_percent- sept_2022$unaff_percent) / sept_2022$unaff_percent * 100)
...
A2. Wicomico county had the largest percentage increase in unaffiliated voters. There was an increase of 70 percent of unaffiliated voters in Wicomico county, and the eighth-largest percentage increase was Caroll county with a 43 percentage increase. 

```{r}

registration_changes2024 <- registration_changes2024 %>% mutate(unaff_percent2024 = (UNAF / TOTAL) * 100)
registration_changes2022 <- registration_changes2022 %>% mutate(unaff_percent2022 = (UNAF / TOTAL) * 100)

merged_changes <- left_join(registration_changes2022, registration_changes2024, by = "COUNTY")

merged_changes <- merged_changes |>
  mutate(unaff_percent_change = ((unaff_percent2024 - unaff_percent2022) / unaff_percent2022) * 100) |>
  arrange(desc(unaff_percent_change))
...
A3. No they did not mirror overall partisan composition of the electorate, more new voters registered as unaffiliated instead of democrats or republicans. Most registrations occurred at the motor vehicle administration in 2022 and 2024. This would be the most interesting trend to further analyze, along with online registration trends over the years. 
I used Chatgpt to help with this question, specifically to see how to properly use the summarize function. 

```{r}

summary2024 <- current_registrations2024 |>
  summarize(
    dem_percent = sum(DEM) / sum(TOTAL) * 100,
    rep_percent = sum(REP) / sum(TOTAL) * 100,
    unaff_percent = sum(UNAF) / sum(TOTAL) * 100
  )

summary2022 <- current_registrations2022 |>
  summarize(
    dem_percent = sum(DEM) / sum(TOTAL) * 100,
    rep_percent = sum(REP) / sum(TOTAL) * 100,
    unaff_percent = sum(UNAF) / sum(TOTAL) * 100
...
new_reg2024 <- new_party_registrations2024 |>
  summarize(
    dem_percent = sum(DEM) / sum(TOTAL) * 100,
    rep_percent = sum(REP) / sum(TOTAL) * 100,
    unaff_percent = sum(UNAF) / sum(TOTAL) * 100
...
new_reg2022 <- new_party_registrations2022 |>
  summarize(
    dem_percent = sum(DEM) / sum(TOTAL) * 100,
    rep_percent = sum(REP) / sum(TOTAL) * 100,
    unaff_percent = sum(UNAF) / sum(TOTAL) * 100
...
A4.What was the total number of removals for Republicans in 2022 and 2024? There were more removals for Republicans in 2022 than in 2024, with 7544 in 2022 and 6406 in 2024. 

```{r}
total_rep_removed2024 <- sum(removals2024$REP, na.rm = TRUE)
total_rep_removed2022 <- sum(removals2022$REP, na.rm = TRUE)

print(total_rep_removed2022)
print(total_rep_removed2024)
...
A5.
The best story idea is why there was such a large increase in unaffiliated voters from 2022 to 2024, what caused this? 