...
**A1.** Election day voting decreased in 2020 and then rose in 2022, after the COVID-19 pandemic. 


Link: https://datawrapper.dwcdn.net/l6KpU/1/ 
...
md_votes_pct <- md_votes |>
  ...
  select(Year, election_day_pct, early_pct, mail_pct)
...
**A2.** I chose this CSV file because I wanted to look into the different trends of voting turnout in regards to gender. I found a trend where less women were voting on day's 3 and 4, which I showed in my chart. 

Question: Why did early voting turnout decrease on day's 3 and 4? 
Link: https://datawrapper.dwcdn.net/KkBTf/1/


```{r}

women_votes <- read_csv("data/ev_totals_women.csv")

women_votes <- women_votes |>
  select(Day, `Montgomery`, `Prince George's`, `Baltimore County`, `Anne Arundel`, `Baltimore City`) 

write_csv(women_votes, "data/ev_totals_women.csv")
...
**A3.** 
Winred Link: https://datawrapper.dwcdn.net/9z8Zy/1/ 
Act Blue Link: https://datawrapper.dwcdn.net/NBH7X/1/

The most interesting story for the ActBlue map is that Oxford, a tiny town in Maryland with only 600 people, was the highest zip code per donation rates. The Winred map shows that Port Tobacco, a town with only 25 people, was the highest zip code. I think this is the most interesting - I would want to look further into the incomes of individuals in these areas, along with any other population information. I would compare these donation rates with areas in Maryland that have larger total populations, such as Montgomery County or Prince George's County. 
...
joined_winred <- winred_zip |>
  left_join(population, by = "zip") |>
  mutate(donations_pct = (donations/population) * 1000)

joined_actblue <- actblue_zip |>
  left_join(population, by="zip") |>
  mutate(donations_pct = (donations/population) * 1000)

joined_winred <- joined_winred |> drop_na(population, donations_pct)
joined_actblue <- joined_actblue |> drop_na(population, donations_pct)

write_csv(joined_winred, "data/md_winred_zip_totals.csv")
write_csv(joined_actblue, "data/md_actblue_zip_totals.csv")
