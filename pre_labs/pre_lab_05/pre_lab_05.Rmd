...
Yes, these five CSV files have the same column structure. 
...
There are 24 rows now. There should be 24 rows after joining the data together. 
...
The output message says the data is joined with 'by = join_by(COUNTY).' This means that the code below used the county column to match the data and perform a join. 
...
The county with the highest 18+ Black population is Prince George's at 60.9%. Prince George's also has a 78.3% of Democrats in the county. 
...
The county with the highest 18+ Asian population is Howard at 19.4%. The population is 52.4% Democrat. The results from both tasks show a higher democratic population than populations of Black or Asian individuals. There is also a large difference in the percentages of Black and Asian individuals in the two counties, with 19% compared to 60%. 
