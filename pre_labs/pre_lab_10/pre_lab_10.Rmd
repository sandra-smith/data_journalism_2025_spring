...
**Answer** The columns that exist in this dataset are precinct names and ID's, congress, legis, councils, school, polling id's, and then shapes with their lengths and areas. This all represents county election data with its geographic locations in PG County. 
...
**Answer** The code displays the pg_precincts dataset. Under the shape column, you can select each different input to show the information about the precinct's geography. 
...
**Answer**The output shows the precincts with their latitudes and longitudes. 
...
**Answer**The output shows that the tigris package was installed. 
...
**Answer**The output shows data from the year 2024, with county names, their geometyt and id's. 
...
**Answer**There is now a dataframe that only shows information on Maryland counties. 
...
**Answer**The output is a map of the Maryland counties with their latitudes and longitudes. 
...
**Answer**This is a map of the precincts and counties in Maryland, the boundaries are different than the original precinct map. 
...
**Answer**The output shows the total voter turnout in PG county by party and precinct. 
...
**Answer** The two dataframes, pg_precincts and pg_turnout are now joined - so we see the voter information in the pg_precint dataset. 
...
**Answer**The new map shows PG county's total eligible voters - color is used to show the differences throughout each precinct. 
...
**Answer**This dataset shows the total eligible voters by counties in Maryland, based on party affiliation. 
...
**Answer**This shows the population estimates for each county in Maryland. 
...
**Answer**The output shows data from 2018-2022 that consists of population estimates, geometry and id's. 
...
**Answer**The voters by county are now shown in alphabetical order by county name. 
...
**Answer**The name column is now called county, and the name is titlecase. There are no whitespaces to prevent a join of the data, and the population column is now called population instead of estimate. 
...
**Answer**The datasets are now joined, so we see population data, geometry data and the total number of eligible voters by county. 
...
**Answer**The names of St. Mary's and Baltimore County are now mutated to match the original dataframe, which removes the NA variables. 
...
**Answer**THe number of voters per 10,000 is now calculated and arranged from highest to lowest. 
...
**Answer** We now have a choropleth map that shows the highest to lowest voters per 10k with color. 
...
**Answer**The colors now changed to make it easier to distinguish the trends of voters. 
...
**Answer**A log scale is now used and we can see the differences more clearly. There are more eligible voters where populations are larger. 
...
**Answer** There is now a column that shows the unaffiliated voters per 10k. 
...
**Answer**We can see that Montgomery County and similar areas have the highest number of unaffiliated voters, while more areas such as Baltimore City and Garrett County have the least. This could have to do with economic status and population trends. 
