...
**Answer** This code is selecting certain columns to keep, including office_name, office_district, candidate_name, party, county_name, election_day, and votes. Then, the mutate function is combining columns (election_day and votes) to create percent_election_day which represents the proportion of votes casted on election day. 
...
**Answer**The results changed because the column is now ordered by descending numbers, meaning to highest amount of votes casted on election day are shown at the top and the lowest amount of votes casted on election day are shown at the bottom. These are also shown as percentages instead of decimals. 
...
**Answer**I think ordering the information in a descending way is more interesting. You can see the counties that had 100% of votes casted for each candidate compared to only seeing how there is zero percent of votes casted. 
...
**Answer**One tidyverse function similar to str_to_upper is str_to_sentence. This function converts to sentences with only the first letter being capitalized. Another function similar to str_to_upper is str_to_title which converts to title, where each word had the first letter capitalized. (Information found from https://stringr.tidyverse.org/reference/case.html)
...
**Answer**The code is creating a new column, amount_category, in the dataset and the case_when function is assigning each variable with an amount. For example, if an amount is <100, it is labeled as Less than $100. It is filtering out the data and organizing it based on amount. 
...
**Answer** Less than 100 at $1716338.60.
...
**Answer**This simplifies the data and makes it easier to understand and analyze. 
...
**Answer:**Expidenture_date is now characterized as a <date> instead of <chr>. This means that R recognizes it as a date and formats each row in a consistent way. 
...
**Answer**The values are showing the total amount of expenditures grouped by month. It is sorted by descending so we see that June 2022 had the highest amount of expenditures. 
