...
Proper grouping is being prevented because there are different spellings, capitalizations and spacings of each name. For example, "W. Millar & Co." is also seen as "W. MILLAR & CO". 
...
One change that was made by the refinr package is that the recipient name "SCARLET OAK" was changed to "Scarlet Oak". It also merged all Capitol Hill Club inputs into uppercase versions. 
...
Walgreens is now combined into one group - "WALGREENS", Walmart is now "WALMART" and Hilton Washington is now "WASHINGTON HILTON." 
...
Yes, the data is now almost all uppercase and combined. Previously, the data was not as clean and all of the data was not grouped together as much as it could have been. The information is now easier to read and is merged together based on each name.
...
```{r}
dc_food_openrefine <- read_csv("data/dc-food-csv.csv")
