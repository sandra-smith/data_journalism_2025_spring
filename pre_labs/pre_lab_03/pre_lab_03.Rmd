...
The warning message says "One or more parsing issues, call 'problems()' on your data frame for details, e.g.: dat<-vroom(...)". 
...
There are 1,640 errors. The problem that R is describing is that it did not expect numerical values, instead it expected characters or words such as true or false. 
...
No, it does not show any parsing errors when the code is run. 
...
The district field is <dbl> or double. The precinct field is <chr> or character, so the column can hold text inputs instead of numerical values. The mail column is also <dbL> or double. 
...
There are 402345 results when mail is NA. This implies that there are missing values. 
...
There are 12 registrations from January 1900. The first 20 results could suggest that there's either data entry errors and it shows that there is a spike in 1966 registrations. 
...
Yes these are worthy of exploration. I think Legacy Data is interesting to me, I'm not sure what that means and the number of these is high. Also, the number of deceased registered voters is high, along with Confirmation not Returned - are the addresses not being updated as frenquently as they should within these results?
...
There are 14 rows in this demonstration data set. 
...
The amount field is <chr> or character. 
...
**Answer** The zip code column consists of both 5 digit zip codes and nine digit zip codes, the column names aren't clean (ex. 1_linenumber) and shouldn't start with numbers, and the amount column is a <chr> data type even though there are numerical values as results. 
...
**Answer**The function changes the name to "x1_linenumber". 
...
**Answer** The error says "Error in summarise(): in argument: 'total_amount = sum(amount)'. caused by error in 'sum()' invalid 'type' (character) of argument. This error means that the amount column's data type is <chr> even though the inputs are numerical. It is reading the data as character strings. 
...
**Answer** There is one duplicate rows. The donor name is Derrick Hamilton. 
...
**Answer**There are 13 rows in the new dataframe. 
...
**Answer**The zip codes are now all five digits. 
...
**Answer**There are two misspellings of Conowingo. 
...
**Answer**There is one misspelling after running this codeblock.
...
**Answer** There are no more misspellings of Conowingo after running this codeblock. 
