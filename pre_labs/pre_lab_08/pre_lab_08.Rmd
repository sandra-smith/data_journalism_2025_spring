...
There are 24 rows and 18 columns in this dataset. 
...
The mutate function is adding the UNAF_DIFF column to the md_voters dataframe. The UNAF_DIFF is made up of the 2024 unaffiliated voters subtracted by the 2022 unaffiliated voters. Then, a new dataframe is created, maryland_top_counties, that the arrange function sets to descending (from largest to smallest unaffiliated difference). Then, the code selects only the county and unaff_diff columns for the data frame. The head function extracts the top 10 rows from the data. The new dataframe is then displayed. 
...
After running the code, there is a gray box displayed. 
...
After running the code, there is now a bar chart that displays the county names on the x axis and the count (unaff_diff) on the y axis. 
...
The bars are now reordered, showing the smallest change to the largest change in unaffiliated voters. 
...
This is now a horizontal bar chart with the axes flipped. 
...
The background is no longer gray and the axis labels are more spaced out.
...
The background is a light blue and the lines changed colors. The labels on the y axis are further away from the bars as well. 
...
There is a title, and the labels on the axes changed. (For some reason the labels are still switched around after running this code?)
...
The code displays dates and total contributions on that day. 
...
There is a basic line chart with total contributions on the y axis and dates on the x axis. 
```{r}
md_winred_by_date |>
  ggplot() + 
...
The dates on the x axis changed to show some days instead of months. 
...
The x axis labels are now on an angle and are easier to read. 
